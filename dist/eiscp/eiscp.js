...
'use strict';
var self, eiscp, send_queue, net = require('net'), dgram = require('dgram'), util = require('util'), async = require('async'), events = require('events'), eiscp_commands = require('./eiscp-commands.json'), COMMANDS = eiscp_commands.commands, COMMAND_MAPPINGS = eiscp_commands.command_mappings, VALUE_MAPPINGS = eiscp_commands.value_mappings, MODELSETS = eiscp_commands.modelsets, config = { port: 60128, reconnect: true, reconnect_sleep: 5, modelsets: [], send_delay: 500, verify_commands: true };
...
function iscp_to_command(iscp_message) {
    ...
    */
    var command = iscp_message.slice(0, 3), value = iscp_message.slice(3), result = {};
...
function command_to_iscp(command, args, zone) {
    ...
    */
    var base, parts, prefix, value, i, len, intranges, default_zone = 'main';
...
self.discover = function () {
    ...
    */
    var callback, timeout_timer, options = {}, result = [], client = dgram.createSocket('udp4'), argv = Array.prototype.slice.call(arguments), argc = argv.length;
    ...
    client
        ...
        var message = eiscp_packet_extract(packet), command = message.slice(0, 3), data;
        if (command === 'ECN') {
...
self.connect = function (options) {
    ...
    */
    ...
    eiscp.
        ...
        var iscp_message = eiscp_packet_extract(data), result = iscp_to_command(iscp_message);
...
self.get_command = function (command, callback) {
    ...
    */
    var val, zone, result = [], parts = command.split('.');
    if (parts.length !== 2) {
        zone = 'main';
        command = parts[0];
    }
